...
def process_image(image: Union[dict, Image.Image, BytesIO]) -> Image.Image:
    ...
    if isinstance(image, dict):
        ...
        elif "bytes" in image and image["bytes"] is None:
            image.pop("bytes")
        if "image_url" not in image and "path" in image:
            path = image["path"]
            if isinstance(path, str):
                if path.startswith("http://") or path.startswith("https://") or path.startswith("file://"):
                    image["image_url"] = path
                else:
                    image["image_url"] = f"file://{os.path.abspath(path)}"
