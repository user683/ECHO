...
class RLHFDataset(Dataset):
    ...
    def __getitem__(self, item):
        ...
        if self.processor is not None:
            ...
            # Keep a local copy for computing position ids; we will drop it from training inputs later.
        ...
        if (
            ...
            and hasattr(self.processor.image_processor, "merge_size")
            and (
                (isinstance(model_inputs, dict) and model_inputs.get("image_grid_thw") is not None)
                or (isinstance(model_inputs, dict) and model_inputs.get("video_grid_thw") is not None)
            )
        ):
            try:
                ...
                position_ids = [
                    get_rope_index(
                        ...
                        image_grid_thw=model_inputs.get("image_grid_thw") if isinstance(model_inputs, dict) else None,
                        video_grid_thw=model_inputs.get("video_grid_thw") if isinstance(model_inputs, dict) else None,
                ...
                logger.warning(
                    "Failed to compute mRoPE position_ids for VLM input; fallback to 1D position_ids. Error: %s", e
                )
                position_ids = None
