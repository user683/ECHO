...
class DataParallelPPOActor(BasePPOActor):
            ...
            if self.use_remove_padding:
                ...
                if self.use_fused_kernels:
                    ...
                    if calculate_confidence:
                        confidence = torch.exp(log_probs).detach()
                    ...
                    confidence = None
                    if calculate_confidence:
                        confidence = self._compute_topk_confidence(logits, confidence_topk)
                    if calculate_entropy:
                        entropy = verl_F.entropy_from_logits(logits)  # (bsz, response_length)

            return entropy, log_probs, confidence

    def _compute_topk_confidence(self, logits: torch.Tensor, topk: int) -> torch.Tensor:
        topk = max(1, min(topk, logits.size(-1)))
        top_logits, _ = torch.topk(logits, k=topk, dim=-1)
        logsumexp = torch.logsumexp(logits, dim=-1, keepdim=True)
        topk_log_probs = top_logits - logsumexp
        confidence = torch.exp(topk_log_probs).mean(dim=-1)
    ...
    def compute_log_prob(
        self, data: DataProto, calculate_entropy=False, calculate_confidence=False, confidence_topk=5
    ) -> torch.Tensor:
        ...
        for micro_batch in micro_batches:
            ...
            if calculate_confidence:
                confidence_lst.append(
                    confidence if confidence is not None else torch.exp(log_probs).detach()
                )
        ...
        confidences = None
        if calculate_confidence and len(confidence_lst) > 0:
        ...
        if use_dynamic_bsz:
            ...
            if calculate_confidence and confidences is not None:
                confidences = confidences[revert_indices]

        return log_probs, entropys, confidences
    ...
    def update_policy(self, data: DataProto):
        ...
        if "clip_epsilon" in data.batch.keys():
            select_keys.append("clip_epsilon")
        batch = data.select(batch_keys=select_keys).batch
        ...
        for epoch in range(self.config.ppo_epochs):
            for batch_idx, data in enumerate(dataloader):
                ...
                for data in micro_batches:
                    ...
                    if "clip_epsilon" in data:
                        clip_tensor = data["clip_epsilon"].unsqueeze(-1).expand_as(response_mask).to(log_prob.device)
                        cliprange_tensor_low = clip_tensor
                        cliprange_tensor_high = clip_tensor
                    ...
                    if self.config.policy_loss.loss_mode == "vanilla":
                        pg_loss, pg_clipfrac, ppo_kl, pg_clipfrac_lower = compute_policy_loss(
                            ...
                            cliprange_tensor_low=cliprange_tensor_low,
                            cliprange_tensor_high=cliprange_tensor_high,
                        )
                    else:
