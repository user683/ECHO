...
def compute_advantage(
    ...
    if adv_estimator == AdvantageEstimator.GAE:
        ...
        data.batch["advantages"] = advantages
        data.batch["returns"] = returns
    data = core_algos.apply_hybrid_advantage_shaping(data, config)
    return data
...
class RayPPOTrainer:
    ...
    def __init__(
        ...
        self._hybrid_adv_cfg = getattr(self.config.algorithm, "hybrid_adv", None)
    ...
    def _hybrid_advantage_enabled(self) -> bool:
        cfg = self._hybrid_adv_cfg
        if cfg is None:
            return False
        try:
    ...
    def _maybe_request_confidence(self, batch: DataProto):
        if not self._hybrid_advantage_enabled():
            return
        cfg = self._hybrid_adv_cfg or {}
        default_topk = 5
        try:
            confidence_topk = cfg.get("confidence_topk", default_topk)
        except AttributeError:
            confidence_topk = getattr(cfg, "confidence_topk", default_topk)
        if batch.meta_info is None:
            batch.meta_info = {}
        batch.meta_info["calculate_confidence"] = True
        batch.meta_info["confidence_topk"] = confidence_topk
    ...
    def _validate(self):
        ...
        for test_data in self.val_dataloader:
            ...
            branch_counts = test_output_gen_batch.meta_info.pop("branch_counts", None)
            if branch_counts is not None and len(branch_counts) != len(test_batch):

                branch_counts = None
            if branch_counts is not None:
                test_batch = test_batch.repeat_by_counts(branch_counts)
            else:
        ...
        validation_metrics = self.config.trainer.get("validation_metrics", "full")
        metric_dict = {}
        if validation_metrics == "basic":
            for var_name, vals in reward_extra_infos_dict.items():
                ...
                for data_source in np.unique(data_sources):
                    ...
                    if not mask.any():
                        continue
                    metric_sec = "val-core" if var_name in ["reward", "acc"] else "val-aux"
                    metric_key = f"{metric_sec}/{data_source}/{var_name}/mean@{val_n}"
                    metric_dict[metric_key] = float(np.mean(vals_arr[mask]))
        else:
        ...
        for epoch in range(self.config.trainer.total_epochs):
            for batch_dict in self.train_dataloader:
                ...
                with marked_timer("step", timing_raw):
                    ...
                    branch_counts = gen_batch_output.meta_info.pop("branch_counts", None)
                    ...
                    if branch_counts is not None and len(branch_counts) != len(batch):
                        branch_counts = None
                    ...
                    if not repeat_sampling_sglang_grpo:
                        ...
                        if branch_counts is not None:
                            assert len(branch_counts) == len(batch), (
                                "branch_counts length must match batch size when repeating prompts"
                            )
                            batch = batch.repeat_by_counts(branch_counts)
                        else:
                            batch = batch.repeat(repeat_times=self.config.actor_rollout_ref.rollout.n, interleave=True)
                    elif branch_counts is not None:
                        batch = batch.repeat_by_counts(branch_counts)
                    ...
                    with marked_timer("old_log_prob", timing_raw, color="blue"):
                        self._maybe_request_confidence(batch)
                        old_log_prob = self.actor_rollout_wg.compute_log_prob(batch)
                        need_hybrid_stats = self._hybrid_advantage_enabled()
                        ...
                        if not need_hybrid_stats:
                            old_log_prob.batch.pop("entropys", None)
                            old_log_prob.batch.pop("confidences", None)
                    ...
                    with marked_timer("adv", timing_raw, color="brown"):
                        ...
                        if self._hybrid_advantage_enabled():
                            batch.batch.pop("entropys", None)
                            batch.batch.pop("confidences", None)
