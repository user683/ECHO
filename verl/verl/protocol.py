...
class DataProto:
    ...
    def repeat_by_counts(self, counts: Union[List[int], np.ndarray]):
        """
        Repeat each sample by the corresponding count.

        Args:
            counts: Number of repetitions for each element in the batch.
        """
        if counts is None:
            raise ValueError("counts must not be None")
        counts = np.asarray(counts, dtype=np.int64)
        if len(counts) != len(self):
            raise ValueError(f"counts length {len(counts)} mismatches batch size {len(self)}")
        ...
        if self.batch is not None:
            ...
            for key, tensor in self.batch.items():
                ...
                if device not in counts_tensor_cache:
                    counts_tensor_cache[device] = torch.as_tensor(counts, device=device, dtype=torch.long)
                repeated_tensors[key] = torch.repeat_interleave(tensor, counts_tensor_cache[device], dim=0)
            repeated_batch = TensorDict(
                source=repeated_tensors,
                batch_size=(int(counts.sum()),),
            )

        repeated_non_tensor_batch = {}
        for key, val in self.non_tensor_batch.items():
            repeated_non_tensor_batch[key] = np.repeat(val, counts, axis=0)

        return type(self)(batch=repeated_batch, non_tensor_batch=repeated_non_tensor_batch, meta_info=self.meta_info)
